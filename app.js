...
(function initCesium() {
  ...
  async function startFlight(originCity, destCity) {
    ...
    viewer.scene.postUpdate.addEventListener(function cameraFollow() {
      ...
      if (pos && nextPos) {
        ...
        const offset = Cesium.Cartesian3.add(
          Cesium.Cartesian3.multiplyByScalar(left, 3000, new Cesium.Cartesian3()),
    ...
    viewer.camera.flyTo({
      destination: Cesium.Cartesian3.fromDegrees(destCity.lon, destCity.lat, 1800),
  ...
  viewer.camera.flyTo({
    destination: Cesium.Cartesian3.fromDegrees(67.1608, 24.9065, 2000),
    orientation: { heading: Cesium.Math.toRadians(0), pitch: Cesium.Math.toRadians(-20), roll: 0 },
    duration: 0
  });
