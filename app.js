...
(function initCesium() {
  ...
  async function startFlight(originCity, destCity) {
    ...
    viewer.scene.postUpdate.addEventListener(function cameraFollow() {
      ...
      if (pos && nextPos) {
        ...
        const offset = Cesium.Cartesian3.add(
          ...
          Cesium.Cartesian3.multiplyByScalar(up, 2500, new Cesium.Cartesian3()),
          new Cesium.Cartesian3()
        );
    ...
    viewer.camera.flyTo({
      destination: Cesium.Cartesian3.fromDegrees(destCity.lon, destCity.lat, 2500),
      orientation: { heading: Cesium.Math.toRadians(0), pitch: Cesium.Math.toRadians(-45), roll: 0 },
      duration: 2
    });
  ...
  viewer.camera.flyTo({
    destination: Cesium.Cartesian3.fromDegrees(67.1608, 24.9065, 2500),
    orientation: { heading: Cesium.Math.toRadians(0), pitch: Cesium.Math.toRadians(-45), roll: 0 },
    duration: 0
  });
